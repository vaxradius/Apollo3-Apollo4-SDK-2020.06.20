...
void llcp_packet_process(uint8_t *llcp_pkt, uint8_t len)
{
    ...
    switch(llcp_opcode)
    {
        ...
        case LL_CONNECTION_PARAM_RSP_OPCODE:
        {
            ...
            am_util_debug_printf("<%s>, param: InterMax:%d,InterMin:%d,Lat:%d,Timout:%d,PrePrid:%d, RefCon:%d\n",llcp_opcode_name[llcp_opcode], 
        ...
        case TRACE:
        {
            ...
            switch(trace_type)
            {
                ...
                case CONN_EVT_CNT:
                {
                    ...
                    am_util_debug_printf("[%02d:%02d:%02d:%03d.%03d] <%s>, conhdl:0x%x,value:0x%x\n", 
                ...
                case RX_DESC:
                {
                    ...
                    am_util_debug_printf("[%02d:%02d:%02d:%03d.%03d] <%s>, rx desc:%s, len:%d\n",
                                        hour, min, sec, ms_dis, us, trace_type_name[trace_type], rx_desc, BLE_RX_DESC_SIZE);
                }
                break;
